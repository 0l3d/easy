...
void
labelsp(const char *label_name, int pos, LabelType type)
{
...
void
datasp(const char *data_label_name, int pos, DataType type)
{
...
void
bsssp(const char *bss_label_name, int pos, BssType type, int size)
{
...
uint16_t
en_registers(const char *reg_name)
{
    int index = -1;
    uint8_t main_type = 0;
...
int
get_label_addr(const char *name)
{
...
void
def_operand(char **tokenized, Instruction *instrc, uint8_t opcode)
{
    ...
    if (tokenized[2][0] != ',') {
        ...
        printf
            ("Warning: same register? Line: %d. Situation: %s -> %s on %s\n",
    ...
    if (isalpha(tokenized[1][0]) || tokenized[1][0] == '\'') {
        if (isalpha(tokenized[1][1])) {
            if (tokenized[1][0] == '\'') {
                if (tokenized[1][2] != '\'') {
                    printf
                        ("Got an error Line: %d, Situation: 'string error' on %s",
                         linecounter, tokenized[0]);
                    return;
                }
                instrc->src = 0xFFFF;
                ...
                if (addr == -1) {
                    ...
                    if (addr == -1) {
                        addr = get_label_addr(tokenized[1]);
                        if (addr == -1) {
                            printf
                                ("Got an error Line: %d, Situation: 'undefined label' on %s",
                                 linecounter, tokenized[0]);
                            return;
                        }
                    }
...
int byte_offset = 0;
int curr_byte_offset = 0;

void
lbl_operand(char **tokens, Instruction *instrc, uint8_t opcode)
{
    instrc->opcode = opcode;
    ...
    if (label_addr == -1) {
        if (isalpha(tokens[1][0])) {
            ...
            if (instrc->dst == 255) {
                printf
                    ("Got an error Line: %d, Sitation: 'undefined label or register' "
                     "%s\n", linecounter, label);
            }
        }
    }
...
void
nlbl_operands(char **tokens, Instruction *instrc, uint8_t opcode)
{
    instrc->opcode = opcode;
    if (isalpha(tokens[1][0])) {
        instrc->dst = en_registers(tokens[1]);
...
void
nothing_operands(Instruction *instrc, uint8_t opcode)
{
    instrc->opcode = opcode;
    instrc->dst = 0xFFFF;
    instrc->src = 0xFFFF;
...
void
nlblnorg_operands(char **tokens, Instruction *instrc, uint8_t opcode)
{
    instrc->opcode = opcode;
    instrc->imm64 = (uint64_t) strtol(tokens[1], NULL, 0);
    instrc->dst = 0xFFFF;
...
void
revdef_operand(char **tokenized, Instruction *instrc, uint8_t opcode)
{
    if (tokenized[2][0] != ',') {
        printf("Got an error on Line %d, Situation: ',' undefined on %s\n",
               linecounter, tokenized[0]);
...
int
opcode(char *tokenized[], Instruction *instrc)
{
    if (!tokenized[0]) {
        return -1;
    }
    if (strcmp(tokenized[0], "nop") == 0) {
    ...
    else if (strcmp(tokenized[0], "inb") == 0) {
        def_operand(tokenized, instrc, OPCODE_INB);
        return 1;
    }
    else if (strcmp(tokenized[0], "outb") == 0) {
        def_operand(tokenized, instrc, OPCODE_OUTB);
        return 1;
    }
    else if (strcmp(tokenized[0], "add") == 0) {
        def_operand(tokenized, instrc, OPCODE_ADD);
        return 1;
    }
    else if (strcmp(tokenized[0], "sub") == 0) {
        def_operand(tokenized, instrc, OPCODE_SUB);
        return 1;
    }
    else if (strcmp(tokenized[0], "mul") == 0) {
        def_operand(tokenized, instrc, OPCODE_MUL);
        return 1;
    }
    else if (strcmp(tokenized[0], "div") == 0) {
        def_operand(tokenized, instrc, OPCODE_DIV);
        return 1;
    }
    else if (strcmp(tokenized[0], "and") == 0) {
        def_operand(tokenized, instrc, OPCODE_AND);
        return 1;
    }
    else if (strcmp(tokenized[0], "or") == 0) {
        def_operand(tokenized, instrc, OPCODE_OR);
        return 1;
    }
    else if (strcmp(tokenized[0], "xor") == 0) {
        def_operand(tokenized, instrc, OPCODE_XOR);
        return 1;
    }
    else if (strcmp(tokenized[0], "not") == 0) {
        def_operand(tokenized, instrc, OPCODE_NOT);
        return 1;
    }
    else if (strcmp(tokenized[0], "shl") == 0) {
        def_operand(tokenized, instrc, OPCODE_SHL);
        return 1;
    }
    else if (strcmp(tokenized[0], "shr") == 0) {
        def_operand(tokenized, instrc, OPCODE_SHR);
        return 1;
    }
        ...
        def_operand(tokenized, instrc, OPCODE_CMP);
        return 1;
    }
    ...
    else if (strcmp(tokenized[0], "ssdp") == 0) {
        ...
        revdef_operand(tokenized, instrc, OPCODE_STORE);
        return 1;
    }
    return 0;
}
...
void
refresh_header(const char *out_file)
{
...
// pass label functions and sections
int
pass(int count, char **tokens)
{
...
collect_bss(char line[1024])
{
    ...
    if (sections == SECTION_BSS) {
        if (count >= 3 && strcmp(tokens[1], "reb") == 0) {
            ...
            bsssp(tokens[0], header.bss_size, DATA_TYPE_RB, size);
            header.bss_size += size;
        }
    }
    for (int i = 0; i < count; i++) {
        free(tokens[i]);
    ...
    if (count > 0) {
        ...
        while (counteri < count) {
            if (count >= 2 && strcmp(tokens[counteri], "ascii") == 0 ||
                strcmp(tokens[counteri], "ascii") == 0) {
                if (strcmp(tokens[0], "ascii") != 0) {
                    datasp(tokens[0], header.bss_size + header.data_size,
                           DATA_TYPE_ASCII);
                }
            ...
            else if (count >= 2 && strcmp(tokens[counteri], "byte") == 0 ||
                     strcmp(tokens[counteri], "byte") == 0) {
                ...
                header.data_size += 1;
                     ...
                     strcmp(tokens[counteri], "hword") == 0) {
                ...
                header.data_size += 4;
                     ...
                     strcmp(tokens[counteri], "dword") == 0) {
    ...
    if (sections == SECTION_DATA) {
        ...
        while (counteri < count) {
            if (count >= 2 && strcmp(tokens[counteri], "ascii") == 0) {
                ...
                if (isdigit(tokens[++counteri][0])) {
                    int digit = atoi(tokens[counteri]);
                    fputc(digit, outfile);
                }
                else {
                    fputc(tokens[counteri][1], outfile);
                }
            }
            else if (count >= 2 && strcmp(tokens[counteri], "hword") == 0) {
    ...
    if (count > 0) {
        if (strcmp(tokens[0], "section") == 0
            ...
            if (!header.section_code) {
                ...
                curr_byte_offset = byte_offset;
                sections = SECTION_CODE;
            }
        }
        if (sections == SECTION_CODE) {
            if (count >= 2 && strcmp(tokens[1], ":") == 0) {
                labelsp(tokens[0], byte_offset, LABEL_TYPE_CODE);
            }
            if (count >= 1 && tokens[0] && strcmp(tokens[0], "section") != 0
                && strcmp(tokens[0], "import") != 0 && (count < 2
                                                        || (tokens[1]
                                                            &&
                                                            strcmp(tokens[1],
                                                                   "ascii") !=
...
int
writer_code(char line[1024], FILE *outfile)
{
    char *tokens[10];
    int count = tokenizer(line, tokens, 10);
    if (count > 0) {
        ...
        if (opcode(tokens, &instrc)) {
            ...
            curr_byte_offset += sizeof(Instruction);
            header.code_size += sizeof(Instruction);
        }
    }
    else {
        return -1;
...
void
write_code_only(const char *asm_file, const char *out_file)
{
    linecounter = 0;
    FILE *outfile = fopen(out_file, "wb");
    if (outfile == NULL) {
