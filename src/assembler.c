...
void def_operand(char **tokenized, Instruction *instrc, uint8_t opcode) {
    ...
    while (counteri < count) {
      if (count >= 2 && strcmp(tokens[counteri], "ascii") == 0 ||
        ...
        if (strcmp(tokens[0], "byte") != 0) {
          datasp(tokens[0], header.bss_size + header.data_size, DATA_TYPE_BYTE);
        }
        byte_offset += 1;
        header.data_size += 1;
        ...
        if (strcmp(tokens[0], "word") != 0) {
          datasp(tokens[0], header.bss_size + header.data_size, DATA_TYPE_WORD);
        }
        byte_offset += 4;
        header.data_size += 4;
...
int big_endian_support(char **tokens, int count) {
  for (int i = 0; i < count; i++) {
...
void write_code_only(const char *asm_file, const char *out_file) {
  linecounter = 0;
  FILE *outfile = fopen(out_file, "wb");
  if (outfile == NULL) {
    perror("append out file failed");
